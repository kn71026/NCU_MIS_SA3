...
public class RecordController extends HttpServlet {
    ...
    private RecordHelper oh = RecordHelper.getHelper();
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!id.isEmpty()) {
            ...
            /** 透過 RecordHelper 物件之 getAll() 方法取回所有訂單之資料，回傳之資料為 JSONObject 物件 */
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        /** 建立一個新的訂單物件 */
        Record od = new Record(first_name, last_name, email, address, phone);
        ...
        for (int i = 0; i < item.length(); i++) {
            ...
            od.addRecordProduct(pd, amount);
        }

        /** 透過 RecordHelper 物件的 create() 方法新建一筆訂單至資料庫 */
        JSONObject result = oh.create(od);

        /** 設定回傳回來的訂單編號與訂單細項編號 */
        od.setId((int) result.getLong("Record_id"));
        od.setRecordProductId(result.getJSONArray("Record_product_id"));

        /** 新建一個 JSONObject 用於將回傳之資料進行封裝 */
        JSONObject resp = new JSONObject();
        resp.put("status", "200");
        resp.put("message", "訂單新增成功！");
        resp.put("response", od.getRecordAllInfo());
