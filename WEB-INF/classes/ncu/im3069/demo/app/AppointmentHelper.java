...
public class AppointmentHelper {
    ...
    public JSONObject create(Appointment a) {
        ...
        try {
            ...
            /** SQL指令 */
            String sql = "INSERT IGNORE INTO `sa_project`.`appointment`(`name`, `pid`,`dob`,`visited_date`,`clinic_hours`,`done`)"
                    + " VALUES(?, ?, ?, ?, ?, ?) ";
    ...
    public JSONObject getByPID(String pid) {
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "SELECT * FROM `sa_project`.`appointment` WHERE `pid` = ? LIMIT 1";
     ...
     */
    public JSONObject update(Appointment a) {
        /** 紀錄回傳之資料 */
        JSONArray jsa = new JSONArray();
        /** 記錄實際執行之SQL指令 */
        String exexcute_sql = "";
        ...
        try {
            ...
            conn = DBMgr.getConnection();
            /** SQL指令 */
            String sql = "Update `sa_project`.`appointment` SET `appointment_number` = ?  WHERE `id` = ?";

            /** 取得所需之參數 */
            int id = a.getID();
            // 過號掛號序號往後移三個
            a.setAppointmentNumber(a.getAppointmentNumber() + 3);
            int appointment_number = a.getAppointmentNumber();

            /** 將參數回填至SQL指令當中 */
            pres = conn.prepareStatement(sql);
            pres.setInt(1, appointment_number + 3);
            pres.setInt(2, id);
